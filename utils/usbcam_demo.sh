...
sleep 8.
byobu send-keys -t 1 'xhost + && docker exec -it detic_2004_container bash -it -c "roslaunch detic_ros detic_segmentor.launch out_debug_img:=true out_debug_segimg:=false compressed:=false device:=auto input_image:=/usb_cam/image_raw"' 'C-m'
sleep 8.
